...
final class ConnectionProbeTests: XCTestCase {
    ...
    func testRefusedCredentialIsReachableAndSaysTheSecretIsTheProblem() {
        for status in [401, 403] {
            ...
            XCTAssertFalse(outcome.isFullyWorking, "but the app still cannot load notes")
    ...
    func testSuccessReportsTheNotesCount() {
        XCTAssertTrue(ProbeOutcome.working(noteCount: 7).message.contains("7"),
                      "a success that names no number proves nothing about the data")
        XCTAssertTrue(ProbeOutcome.working(noteCount: 7).isFullyWorking)
    }
    ...
    /// Pointing at something that answers HTTP but is NOT a healthy notes
    ...
    /// it means the notes service was not running.
    func testRealServiceWithNoSecretReportsUnauthorized() async throws {
        ...
        guard await serviceIsUp(config) else {
            throw XCTSkip("notes service not listening on \(ServerConfig.simulatorURL)")
        }
...
private final class StubURLProtocol: URLProtocol {
    ...
    override func startLoading() {
        ...
        client?.urlProtocol(self, didLoad: Data(#"{"ok":true,"notes":[]}"#.utf8))
